...
// Fetch jobs that have been started (driverStatus != 'not_started'), optionally filtered by driver
if ($selectedDriver && $selectedDriver !== 'all') {
    $stmt = $mysqli->prepare("SELECT * FROM jobs WHERE complete = 0 AND driver = ? AND driverStatus != 'not_started'");
    $stmt->bind_param("s", $selectedDriver);
    $stmt->execute();
    $jobs = $stmt->get_result();
} else {
    $jobs = $mysqli->query("SELECT * FROM jobs WHERE complete = 0 AND driverStatus != 'not_started'");
}
// Fetch drivers and count of started jobs
$drivers = $mysqli->query("
  ...
  FROM users u
  LEFT JOIN jobs j 
    ON j.driver = u.username 
   AND j.complete = 0 
   AND j.driverStatus != 'not_started'
  GROUP BY u.username
...
<head>
  ...
  <title>We-Drive | Active Jobs</title>
...
<main class="col-md-10 ms-sm-auto px-4" style="height: 90vh; overflow-y: hidden;">
  <div class="page-header">
    ...
    <div class="mb-3">
      ...
      <a href="completejobs.php" class="btn btn-danger">Completed Jobs</a>
 ...
 <form method="GET" class="d-flex align-items-center">
    ...
    <select name="driver" id="driver" class="form-select form-select-sm me-2" onchange="this.form.submit()">
      <option value="all" <?= $selectedDriver === 'all' || $selectedDriver === '' ? 'selected' : '' ?>>
        All Drivers (<?= $mysqli->query("SELECT COUNT(*) AS total FROM jobs WHERE complete = 0")->fetch_assoc()['total'] ?>)
      </option>
