...
// Fetch active jobs, filtered by driver if selected
if ($selectedDriver && $selectedDriver !== 'all') {
    ...
    $jobs = $mysqli->query("SELECT * FROM jobs WHERE complete = 0");
}

// Fetch drivers and count of active jobs
$drivers = $mysqli->query("
  SELECT 
    u.username,
    COUNT(j.jobNumber) AS job_count
  FROM users u
  LEFT JOIN jobs j ON j.driver = u.username AND j.complete = 0
  GROUP BY u.username
